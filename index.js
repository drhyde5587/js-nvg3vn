...
function generateShortCode(storeId, transactionId) {
  ...
  var uuid = 'x'.replace(/[xy]/g, function (c) {
    var appendedID =
      '' +
      storeId.toString().length +
      transactionId.toString().length +
      storeId +
      transactionId;
    ...
    if (appendedID.length < 8) {
      var randomnumber =
        Math.floor(Math.random() * (9 - appendedID.length - das[l + 1] + 1)) +
        das[l + 1];
...
function decodeShortCode(shortCode) {
  ...
  return {
    storeId: parseInt(
      shortCode.substring(iLen.length, parseInt(sLen, 10) + iLen.length),
      10
    ), // store id goes here,
    shopDate: new Date(), // the date the customer shopped,
    transactionId: parseInt(
      shortCode.substring(
        parseInt(sLen, 10) + iLen.length,
        iLen.length + parseInt(sLen, 10) + parseInt(tLen, 10)
      ),
      10
